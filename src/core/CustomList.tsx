...
const CustomList: FC<ListOptions> = ({ options, optionAction, readOnly }) => {
    ...
    return (
        <div className="sm:container sm:mx-auto px-4">
            ...
            {current.length > 0 && <ul className='divide-y divide-gray-200 border rounded-lg sm:mx-auto max-w-lg'>
                {current.map(o => <CustomOption key={o.value} name={o.name} value={o.value} action={readOnly ? undefined: optionAction} />)}
            </ul>}
