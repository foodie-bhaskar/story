...
const ItemQty: FC<ItemQtyOtps> = ({ item, qty, readOnly }) => {
    const { itemId, name } = item;
    return (
        <li key={itemId} className="group flex pe-8 py-2 gap-1 rounded max-w-lg transition-opacity-250 duration-250 timing-ease-in-out bg-white">
            
            <div className='w-full y-4 px-2 flex self-center h-10 rounded gap-2'>
                <div className={`${readOnly ? 'text-gray-400 bg-gray-50': 'bg-slate-500 text-white'} w-12 rounded text-center my-auto`}>{itemId}</div>
