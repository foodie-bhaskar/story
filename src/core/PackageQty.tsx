...
const PackageQty: FC<PackageQtyOtps> = (props) => {
    ...
    const { qty } = props;
    return (
        <li key={packageId} className="group flex pe-8 py-2 gap-1 rounded-lg w-md max-w-lg transition-opacity-250 duration-250 timing-ease-in-out bg-white">
            
            <div className='w-full y-4 px-2 flex self-center h-10 rounded gap-2'>
                <div className='w-12 bg-slate-500 text-white rounded text-center my-auto'>{packageId}</div>
